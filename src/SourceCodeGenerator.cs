public class SourceCodeGeneratorCS
{
	...
	void DataClass(StreamWriter sw)
	{
		...
		sw.WriteLine($"");
		sw.WriteLine($"		public List<{mainTable.name}> {mainTable.name}List {{ get => {mainTable.name}s.Values.ToList(); }}");
		sw.WriteLine($"		public bool HasKey({mainTable.keyType} id) => {mainTable.name}s.ContainsKey(id);");
	...
	void DataTableConvertFunc(StreamWriter sw)
	{
		...
		sw.WriteLine("				{");
		sw.WriteLine("#if UNITY_2021_3_OR_NEWER || CLIENT");
